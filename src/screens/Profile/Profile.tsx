...
const Profile: React.FC = () => {
  const { username } = useParams<useParamsTypes>();
  const { data: userData } = useUser();
  const client = useApolloClient();
  const { data, loading } = useSeeProfileQuery({ variables: { username } });

  const unfollowUserUpdate = (
    cache: ApolloCache<UnfollowUserMutation>,
    result?: Omit<FetchResult<UnfollowUserMutation>, 'context'>,
  ) => {
  ...
  const followUserCompleted = (data: FollowUserMutation) => {
    const {
      followUser: { ok },
    } = data;
    if (!ok) {
  ...
  const [followUser] = useFollowUserMutation({ variables: { username }, onCompleted: followUserCompleted });
  const getButton = (seeProfile: SeeProfileQuery['seeProfile']) => {
    if (seeProfile) {
